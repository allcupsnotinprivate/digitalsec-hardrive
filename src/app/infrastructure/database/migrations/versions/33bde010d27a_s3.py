...
def upgrade() -> None:
    op.add_column('documents', sa.Column('storage_bucket', sa.VARCHAR(length=63), nullable=False, server_default=sa.text("'default-bucket'")))
    op.add_column('documents', sa.Column('storage_key', sa.VARCHAR(length=512), nullable=False, server_default=sa.text("gen_random_uuid()::text")))
